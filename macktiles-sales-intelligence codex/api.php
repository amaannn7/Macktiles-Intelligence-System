...
function generateNotifications($leads, $existingNotifications) {
    ...
    foreach ($existingNotifications as $notif) {
        $key = ($notif['type'] ?? '') . '_' . ($notif['lead_id'] ?? '') . '_' . date('Y-m-d', strtotime($notif['created_at'] ?? 'now'));
    ...
    foreach ($leads as $lead) {
        ...
        // Check: Hot lead with no contact in 1+ days
        if (in_array($temp, ['on_fire', 'hot']) && $daysSinceActivity >= 1) {
            $key = "hot_lead_{$leadId}_{$today}";
            if (!isset($existingKeys[$key])) {
                $newNotifications[] = [
                    'id' => 'notif_' . bin2hex(random_bytes(8)),
                    'type' => 'hot_lead',
                    'lead_id' => $leadId,
                    'message' => "🔥 {$name} ({$company}) is hot but hasn't been contacted in {$daysSinceActivity} day(s)",
                    'created_at' => date('c'),
                    'read' => false,
                    'severity' => 'high'
                ];
        ...
        // Check: Callback due today
        if (!empty($lead['followup_date'])) {
            $followupDate = date('Y-m-d', strtotime($lead['followup_date']));
            if ($followupDate === $today) {
                $key = "callback_due_{$leadId}_{$today}";
                if (!isset($existingKeys[$key])) {
                    $newNotifications[] = [
                        'id' => 'notif_' . bin2hex(random_bytes(8)),
                        'type' => 'callback_due',
                        'lead_id' => $leadId,
                        'message' => "📞 Callback due today: {$name} ({$company})",
                        'created_at' => date('c'),
                        'read' => false,
                        'severity' => 'high'
                    ];
            ...
            $followupTime = strtotime($lead['followup_date']);
            if ($followupTime && $followupTime < strtotime('today')) {
                $key = "callback_overdue_{$leadId}_{$today}";
                if (!isset($existingKeys[$key])) {
                    $daysOverdue = floor(($now - $followupTime) / 86400);
                    $newNotifications[] = [
                        'id' => 'notif_' . bin2hex(random_bytes(8)),
                        'type' => 'callback_overdue',
                        'lead_id' => $leadId,
                        'message' => "⚠️ Callback overdue by {$daysOverdue} day(s): {$name} ({$company})",
                        'created_at' => date('c'),
                        'read' => false,
                        'severity' => 'critical'
                    ];
        ...
        // Check: Going cold (was warm/hot, no activity 7+ days)
        if (in_array($temp, ['hot', 'warm']) && $daysSinceActivity >= 7 && $daysSinceActivity < 14) {
            $key = "going_cold_{$leadId}_{$today}";
            if (!isset($existingKeys[$key])) {
                $newNotifications[] = [
                    'id' => 'notif_' . bin2hex(random_bytes(8)),
                    'type' => 'going_cold',
                    'lead_id' => $leadId,
                    'message' => "❄️ {$name} ({$company}) is going cold - no activity in {$daysSinceActivity} days",
                    'created_at' => date('c'),
                    'read' => false,
                    'severity' => 'warning'
                ];
        ...
        // Check: Stale research (research done but no outreach in 5+ days)
        $status = getLeadStage($lead);
        if ($status === 'research' && $daysSinceActivity >= 5) {
            $key = "stale_research_{$leadId}_{$today}";
            if (!isset($existingKeys[$key])) {
                $newNotifications[] = [
                    'id' => 'notif_' . bin2hex(random_bytes(8)),
                    'type' => 'stale_research',
                    'lead_id' => $leadId,
                    'message' => "📋 Research on {$name} ({$company}) done {$daysSinceActivity} days ago - send outreach",
                    'created_at' => date('c'),
                    'read' => false,
                    'severity' => 'warning'
                ];
...
case 'generate-notifications':
    ...
    $newNotifications = generateNotifications($leads, $userData['notifications'] ?? []);

    if (!empty($newNotifications)) {
        $userData['notifications'] = array_merge($userData['notifications'] ?? [], $newNotifications);
        saveUserData($user['id'], $userData);
    }
